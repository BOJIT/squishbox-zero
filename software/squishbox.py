...
SCROLL_TIME = 0.4
SCROLL_PAUSE = 4  # scrolling text options
HOLD_TIME = 1.0
MENU_TIMEOUT = 5.0  # menu timimng options
...
MIDI_CTRL = 2  # MIDI channel for controls
MIDI_DEC = 72  # CC for patch/value decrement
MIDI_INC = 74  # CC for patch/value increment
...
BACKSLASH, TILDE, CHECK, XMARK, SUBDIR, WIFIUP, WIFIDOWN, MIDIACT = [
    chr(i) for i in range(8)
]
FNCHARS = (
    " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_./" + BACKSLASH
)
USCHARS = (
...
class SquishBox:
    ...
    def __init__(self):
        ...
        display.display("Squishbox-Zero")
        ...
        self.wifi_state()
        sys.excepthook = lambda etype, err, tb: self.display_error(
            err, etype=etype, tb=tb
        )
    ...
    def choose_val(
        self, val, minval, maxval, inc, fmt=f">{COLS}", timeout=MENU_TIMEOUT, func=None
    ):
        ...
        while True:
            ...
            while timeout == -1 or time.time() - t < timeout:
                ...
                if event in (DEC, INC):
                    ...
                    if c:
                        self._lcd_flash(text[:COLS], row, mode="ljust")
                    return c
                elif event == ESCAPE:
                    timeout = 0
            else:
    ...
    def char_input(
        self, text=" ", row=ROWS - 1, i=-1, timeout=MENU_TIMEOUT, charset=FNCHARS
    ):
        ...
        while True:
            if mode == "blink":
                self.lcd_write(
                    text[max(0, i + 1 - COLS) : max(COLS, i + 1)],
                    row,
                    mode="ljust",
                    now=True,
                )
    ...
    def choose_file(self, topdir, last="", ext=None):
        ...
        """
        cdir = (
            topdir if last == "" else (last.parent if last.parent > topdir else topdir)
        )
        while True:
            self.lcd_write(
                f"{str(cdir.relative_to(topdir.parent))}/:", ROWS - 2, mode="scroll"
            )
            x = sorted(
                [
                    p
                    for p in cdir.glob("*")
                    if p.is_dir() or p.suffix == ext or ext == None
                ]
            )
            ...
            if self.wifi_state() == "off":
                ...
                if j < 0:
                    ...
                    if x == -1:
                        ...
                        try:
                            x = sb.shell_cmd(
                                f"sudo nmcli dev wifi connect {opts[j][1:]} password {psk}"
                            )
                        except subprocess.CalledProcessError:
                            x = -1
                        self.progresswheel_stop()
...
class FluidBox:
    ...
    def handle_buttonevent(self, val):
        ...
        if val:
            self.buttonstate ^= 1
            fp.send_event(
                type="cc", chan=STOMP_CHAN, par1=STOMP_TOGGLE, par2=self.buttonstate
            )
            if PIN_LED:
    ...
    def listener(self, sig):
        ...
        """
        if "type" in sig and sig.type == "pbend":
            return

        # Custom Patch Control Logic
        if "type" in sig and sig.type == "note":
            if "chan" in sig and sig.chan == MIDI_CTRL:
                if "par1" in sig and sig.par1 == MIDI_DEC:
                    self.pno = (self.pno - 1) % len(fp.patches)
                    display.display(fp.patches[self.pno])
                elif "par1" in sig and sig.par1 == MIDI_INC:
                    display.display(fp.patches[self.pno])
                    self.pno = (self.pno + 1) % len(fp.patches)
        ...
        while True:
            if fp.patches:
                ...
                if warn:
                    ...
                    sb.waitfortap()
                sb.lcd_write(
                    f"patch: {self.pno + 1}/{len(fp.patches)}", 1, mode="rjust"
                )
            else:
    ...
    def save_bank(self, bank=""):
        ...
        try:
            fp.save_bank(bank.with_suffix(".yaml"))
        except Exception as e:
    ...
    def effects_menu(self):
        ...
        fxopts = [
            fxmenu_info[i][0] + ":" + format(vals[i], fxmenu_info[i][5])
            for i in range(len(fxmenu_info))
        ]
        while True:
            ...
            newval = sb.choose_val(
                vals[i],
                *fxmenu_info[i][2:],
                func=lambda x: fp.fluidsetting_set(fxmenu_info[i][1], x),
            )
            if newval != None:
    ...
    def system_menu(self):
        ...
        k = sb.choose_opt(
            ["Power Down", "MIDI Devices", "Wifi Settings", "USB File Copy"], row=1
        )
        if k == 0:
            sb.lcd_write("Shutting down..", 0, mode="ljust")
            sb.lcd_write("Wait 15s, unplug", 1, mode="ljust", now=True)
    ...
    def midi_devices(self):
        ...
        if 0 <= p < len(rports):
            ...
            if op < 0:
                return
            if "midiconnections" not in fp.cfg:
                fp.cfg["midiconnections"] = []
            fp.cfg["midiconnections"].append(
                {rnames[p]: re.sub("(FLUID Synth) \(.*", "\\1", wnames[op])}
            )
            fp.update_config()
            try:
            ...
            while not sb.waitfortap(0.1):
                if self.lastsig == msg or self.lastsig == None:
                    continue
                msg = self.lastsig
                if msg.type not in (
                    "note",
    ...
    def midi_connect():
        ...
        for link in fp.cfg.get("midiconnections", []):
            ...
            try:
                sb.shell_cmd(f"aconnect {mfrom} {mto}")
            except subprocess.CalledProcessError:
                pass
    ...
    def usb_filecopy():
        ...
        try:
            ...
            sb.shell_cmd(
                f"sudo mount -o owner,fmask=0000,dmask=0000 {usb[0]} /mnt/usbdrv/"
            )
            if j == 0:
    ...
    def add_midicontrols():
        ...
        if MIDI_DEC != None:
            fp.add_router_rule(
                type="cc", chan=MIDI_CTRL, par1=MIDI_DEC, par2="1-127", event=DEC
            )
        if MIDI_INC != None:
            fp.add_router_rule(
                type="cc", chan=MIDI_CTRL, par1=MIDI_INC, par2="1-127", event=INC
            )
        if MIDI_PATCH != None:
        ...
        if MIDI_SELECT != None:
            fp.add_router_rule(
                type="cc", chan=MIDI_CTRL, par1=MIDI_SELECT, par2="1-127", event=SELECT
            )
        if MIDI_ESCAPE != None:
            fp.add_router_rule(
                type="cc", chan=MIDI_CTRL, par1=MIDI_ESCAPE, par2="1-127", event=ESCAPE
            )
        if MIDI_SHUTDOWN != None:
            fp.add_router_rule(
                type="cc", chan=MIDI_CTRL, par1=MIDI_SHUTDOWN, shutdown=1
            )
        else:
...
if __name__ == "__main__":
    ...
    sb.lcd_clear()
    try:
        fp = FluidPatcher("SquishBox/fluidpatcherconf.yaml")
    except Exception as e:
